...
$(function(){
	...
	if(Request['action'] == 'doInSearch'){
		url = url + '?t1.type=1';
	}
	if(Request['action'] == 'doOutSearch'){
		url = url + '?t1.type=2';
	}
	...
	$('#itemgrid').datagrid({
		columns:[[
		          ...
		          {field:'state',title:'状态',width:100,formatter:getReturnState},
...
//		    					$('#itemgrid').datagrid('beginEdit',existEditIndex);
//		    		    	}else{
//		    		    		currentEditIndex = rowIndex;
//		    					$('#itemgrid').datagrid('beginEdit',existEditIndex);
//		    		    	}
...
//		    	   			$(editor.target).focus();
		...
		columns:[[    
		      ...
		      onDblClickRow:function(rowIndex,rowData){
		    	  /*	if(Request['action'] == 'doRCreate'){
		    	  		...
		    	  		/*if(Reqeust['action'] == 'doInCheck' || Reqeust['action'] == 'doOutCheck' ){*/
	...
	if(Request['action'] == 'doRInCheck' || Request['action'] == 'doROutCheck'){
	...
	if(Request['action'] == 'doInSearch' || Request['action'] == 'doOutSearch'){
		$('#detailitemDlg').dialog({
			title:'退货订单详情查看',
			closed:true,
			height:250,
	...
	//初始化窗口
	if(Request['action'] == 'doROutStore' || Request['action'] =='doRInStore' ){
		$('#detailitemDlg').dialog({
			title:inOutTitle,
			closed:true,
			height:250,
