...
$(function(){
	...
	var url = 'orders/getListByPage';//所有订单
	//我的订单(myorders)
	if (Request['oper']=='myorders') {//先判断第一层级oper
		if (Request['type']*1==1) {
			url='orders/getListByPage?type=1';
		...
		if (Request['type']*1==2) {
			...
			url='orders/getListByPage?type=2&state=0';
...
//	    	$('#waybillsn').html("");
	    		...
	    		toolbar.push({
	    			...
	    			handler:function(){
	    				...
	    				$('#waybillgrid').datagrid({
	    					url:'orders/waybilldetailList?waybillsn='+$('#waybillsn').html(),
	    					columns:[[
	...
	//明细表格itemgrid
	$('#itemgrid').datagrid({
		//url:'orderdetail_getListByPage?state=0',
		columns:[[
...
function doCheck(){
	$.messager.confirm('确认','是否要审核此订单?',function(yes){
		if (yes) {
			...
			$.ajax({
				//页面上需要一个id(当前订单号)
				url:'orders/doCheck?id='+$('#uuid').html(),//html标签取值.
				dataType:'json',	
				type:'post',
...
function doStart(){
	$.messager.confirm('确认','是否要确认此订单?',function(yes){
		if (yes) {
			//确认订单--->发送ajax请求
			$.ajax({
				url:'orders/doStart?id='+$('#uuid').html(),//html标签取值
				dataType:'json',
				type:'post',
				success:function(rtn){
					//提示确认信息
...
function doInOutStore(){
	...
	if (Request['type']*1==1) {
		//入库
		message ="是否确认入库?";
		url ="orderdetail/doInStore";
	}
	if (Request['type']*1==2) {
		//出库
		message ="是否确认出库?";
		url ="orderdetail/doOutStore";
	}
	//把itemForm表单转json格式
 ...
 */
function doExport(){
	$.download("orders/export",{"id":$("#uuid").html()});
}
