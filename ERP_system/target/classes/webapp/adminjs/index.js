	...
	$.ajax({
		...
		success: function(rtn){
			if (rtn.success) {
				...
				//$('#username').val(rtn.message);
				//$("#username").attr("value",rtn.message);
			} else {
				//没有登录，跳到登录界面
...
//	$("#nav").accordion({animate:false,fit:true,border:false});
	var selectedPanelname = '';
	    ...
	    $.each(_menus.menus, function(i, n) {
			...
			var menulist ='';
			menulist +='<div  class="mainPanle" >';
	        $.each(n.menus, function(j, o) {
				menulist += ' <p><i class="i_a i_low"></i><a ref="'+o.menuid+'" href="javascript:void(0)" rel="' + o.url + '">'+'<span class="nav">' + o.menuname + '</span></a></p>';
				/*
       ...
       ref="'+o.menuid+'" href="javascript:void(0)" rel="' + o.url + '" 
	...
	$('.mainPanle p a').click(function(){
		...
		if(third && third.child && third.child.length>0)
		{
			...
			$('.mainPanle p ').removeClass("selected");
    ...
    $('#w').dialog({
        ...
        buttons:[{
	         ...
	         handler:function(){
	        	 ...
	        	 if (newPwd != rePwd) {
	        		 $.messager.alert('提示','新密码不能不一致！','info');
	        		 return;//还在原来页面
	        	 }
	        	 //数据请求，因为不是form表单
	        	 $.ajax({
