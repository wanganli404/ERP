...
public class GetNameOrSaveUtil {
	 ...
	 */
	/*public static String getGoodsName(Long uuid, Map<Long, String> goodsNameMap, GoodsMapper goodsMapper) {
		if (null == uuid) {
			return null;
		}
	...
	*//**
	 ...
	 * @return empName 员工名称
	 */
	public static String getEmpName(Long uuid, Map<Long, String> empNameMap, EmpMapper empMapper) {
		if (null == uuid) {
			return null;
		}
		// 从缓存中根据员工编号取出员工名称
		...
		if (null == empName) {
			// 如果没有找员工的名称，则进行数据库查询
			empName = empMapper.selectByPrimaryKey(uuid).getName();
			// 存入缓存中
	 ...
	 */
	public static String getSupplierName(Long uuid, Map<Long, String> supplierNameMap,SupplierMapper supplierMapper) {
		// 先判断uuid是否为空
		if (null == uuid) {
			return null;
		}
		...
		if (null == supplierName) {
			...
			supplierName = supplierMapper.selectByPrimaryKey(uuid).getName();
			// 在缓存中保存一份
